...
from utils_FDA import *

# values taken from compute_std_mean.py in step3 folder
IMG_MEAN = np.array((0.5014, 0.4765, 0.4517), dtype=np.float32)
...
def train(args, curr_state, model, model_D, optimizer, optimizer_D, 
    ...
    for epoch in range(curr_epoch, args.num_epochs):
        ...
        for i in range(target_size):
            ...
            src_in_trg = FDA_source_to_target( src_img, trg_img, L=0.01 )
            trg_in_trg = trg_img
            ...
            # normalization by simply substacting the mean
            src_img = src_in_trg.cuda() - mean_img.cuda() #IDDA mean
            trg_img = trg_in_trg.cuda() - mean_img.cuda()

            # train G with source (IDDA)
            with amp.autocast():
                output, output_sup1, output_sup2 = model(src_img)
                loss1 = loss_func(output, src_lbl)
                loss2 = loss_func(output_sup1, src_lbl)
                loss3 = loss_func(output_sup2, src_lbl)
...
def main(params):
    ...
    dataset_train = CamVid(train_path, train_label_path, csv_path, scale=(args.crop_height, args.crop_width),
                           loss=args.loss, mode='train', normalize=False)
    ...
    dataset_val = CamVid(test_path, test_label_path, csv_path, scale=(args.crop_height, args.crop_width),
                         loss=args.loss, mode='test', normalize=False)
